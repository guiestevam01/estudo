...
export interface TransmissionStatus {
  orderId: string;
  transmissionStatus: string | null; // e.g. 'OK', 'SUCCESS', 'ERROR', 'PENDING'
...
// Fetch orders from Anymarket transmissions advanced endpoint. If the
// environment variable `VITE_ANYMARKET_GUMGA_TOKEN` is not set or the
// request fails, return an empty array (no simulator behavior).
export async function fetchOrdersDashboard(): Promise<OrderWithTransmission[]> {
  ...
  if (!gumgaToken) {
    console.warn('VITE_ANYMARKET_GUMGA_TOKEN não definido — não usando simulador; retornando lista vazia');
    return [];
  }
